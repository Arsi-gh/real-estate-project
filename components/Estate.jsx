...
export default function Estate({id , title , images , forRent , location , type , price , rooms , bathrooms , size}) {
  ...
  return (
    <Link href={`/estates/${id}`} className='w-[20rem] h-fit flex flex-col rounded-xl overflow-hidden bg-zinc-100 shadow-customeOne cursor-pointer max-md:w-full max-md:flex-row max-md:p-2 max-sm:rounded-none max-sm:bg-white max-sm:shadow-none max-sm:border-t-[1px] max-[380px]:flex-col border-zinc-300'>
        <img className='rounded-xl h-[12rem] object-cover max-md:h-[8rem] max-md:w-[10rem]' src={images[0]} alt="" />
        <div className='flex flex-wrap items-center gap-2 gap-x-4 p-2 max-md:justify-between'>
            ...
            <span className='w-full h-[1px] bg-zinc-300 my-1 max-sm:hidden'></span>
